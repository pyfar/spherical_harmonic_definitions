...
def balloon_wireframe(
    ...
    """  # noqa: E501
    from spharpy.plot.spatial import _triangulation_sphere
    tri, xyz = _triangulation_sphere(coordinates, data)

    from spharpy.plot.cmap import phase_twilight
    ...
    from spharpy.plot.spatial import _balloon_color_data
...
def plot_basis_functions(Y, sampling, n_max=2):
    ...
    for acn in range((n_max+1)**2):
        ...
        balloon = balloon_wireframe(
            sampling, Y[:, acn], phase=True, colorbar=False,
            ax=ax)
        ax.set_title('$Y_{' + str(n) + '}^{' + str(m) + '}(\\theta, \\phi)$')
        plt.axis('off')
        length_axis = np.max(np.abs(Y[:, acn])) * 1.2

        tri, xyz = spharpy.plot.spatial._triangulation_sphere(
            sampling, np.abs(Y[:, acn]))
        lims = [np.ptp(i) for i in xyz]

        scaling = 1.5

        x = np.linspace(0, 1, 100) * np.max(xyz[0])*scaling
        y = np.linspace(0, 1, 100) * np.max(xyz[1])*scaling
        z = np.linspace(0, 1, 100) * np.max(xyz[2])*scaling

        ax.plot(
            x, np.zeros_like(x), np.zeros_like(x),
            color='k', linestyle='-', alpha=1)
        ax.plot(
            np.zeros_like(y), y, np.zeros_like(y),
            color='k', linestyle='-', alpha=1)
        ax.plot(
            np.zeros_like(z), np.zeros_like(z), z,
            color='k', linestyle='-', alpha=1)

        xyz = np.ones(3)*np.max(np.abs(np.linalg.norm(xyz, axis=0)))
...
n_max = 3
sampling = spharpy.samplings.equalarea(0, n_points=700)
```


```{python}
#| fig-cap: Complex valued spherical harmonics as implemented in scipy.
#| label: fig-complex-sh-scipy
Y_nm = np.zeros((sampling.n_points, (n_max+1)**2), dtype=complex)
for acn in np.arange((n_max+1)**2):
    n, m = spharpy.spherical.acn2nm(acn)
    Y_nm[:, acn] = sph_harm(m, n, sampling.azimuth, sampling.elevation)

axs, gs = plot_basis_functions(Y_nm, sampling, n_max=n_max)
```

[Fig. @fig-complex-sh-scipy] shows the complex valued spherical harmonics as implemented in scipy.


```{python}
theta, phi = sym.symbols(r'\theta \phi', real=True, positive=True)
n, m = sym.symbols('n m', integer=True)
Y_nm_sympy = sym.functions.special.spherical_harmonics.Ynm(n, m, theta, phi)
```

```{python}

from sympy.functions.special.polynomials import assoc_legendre
P_nm_williams = assoc_legendre(n, m, sym.cos(theta))
Y_nm_williams = sym.sqrt((2*n+1)/4/sym.pi * sym.factorial(n-m)/sym.factorial(n+m)) * P_nm_williams * sym.exp(sym.I*phi*m)
